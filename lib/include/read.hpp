    ...
    response<Body> & msg)
{
    ...
    response<string_body> placeholder;
    ...
    std::visit([&](auto && p)
    {
        ...
        if constexpr (std::is_same_v<T, Ringbeller::prompt>)
        {
            msg = p;
        }
        else if constexpr (std::is_same_v<T, Ringbeller::final_result<string_body>>)
        {
            Ringbeller::final_result<Body> fr(p.rc);

            std::copy(p.text.cbegin(), p.text.cend(), std::back_inserter(fr.text));
...
template<
    ...
    typename Body>
std::size_t
read(SyncReadStream & stream, DynamicBuffer & buffer, response<Body> & msg, boost::system::error_code & ec)
{
    FN_ENTER();
